...
def export_temperature_chain_to_json(blockchain, file_path):
    data_list = []
...
def export_precipitation_chain_to_json(blockchain, file_path):
    data_list = []
    ...
    for block in blockchain.chain[1:]:
        ...
        data_list.append({
            "Index": block.index,
            "Timestamp": block.timestamp,
            "Precipitation": block.data['precipitation'],
            "Date": formatted_date.isoformat(),
            "Previous Hash": block.previous_hash,
            "Hash": block.hash
        })
